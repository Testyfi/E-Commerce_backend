...
func GenerateAllAdminTokens(email string, adminName string, id string) (signnedToken string, signedRefreshToken string, err error) {
...
func ValidateAdminToken(signedToken string) (claims *SignedAdminDetails, msg string) {
	token, err := jwt.ParseWithClaims(signedToken, &SignedAdminDetails{}, func(token *jwt.Token) (interface{}, error) { return []byte(SECRET_KEY), nil })
	if err != nil {
		msg = err.Error()
		return
	}
...
func UpdateAllAdminTokens(signedToken string, signedRefreshToken string, AdminId string) {
	var ctx, cancel = context.WithTimeout(context.Background(), 100*time.Second)
